...
DATASETS = {'snli': pd.read_csv(r'datasets\snli.csv'),
...
def pie_by_langs(data):
    ...
    data = DATASETS[data]
    fig = px.pie(values=data.language.value_counts().values, 
             names=data.language.value_counts().index, color_discrete_sequence=px.colors.sequential.matter)
...
def word_count_langwise(data):
    ...
    if data.language.unique().size != 1:
        ...
        for i,lang in enumerate(data.language.unique()): 
            fig.add_trace(go.Histogram(histnorm='density', 
                                       x=data[data.language==lang].hypothesis.apply(lambda x: len(x.split())),
                                       name='hypothesis',
                                       marker_color='#ef767a',
                                       showlegend=(i==0)),
                         row=divmod(i,3)[0]+1,col=divmod(i,3)[1]+1)
            fig.add_trace(go.Histogram(histnorm='density', 
                                       x=data[data.language==lang].premise.apply(lambda x: len(x.split())),
                                       marker_color='#9f87af',
                                       name='premise',
                                       showlegend=(i==0)),
                         row=divmod(i,3)[0]+1,col=divmod(i,3)[1]+1)
        ...
        wc = pd.DataFrame({'word_count': data.hypothesis.apply(lambda x: len(x.split())),
               'type' : 'hypothesis'}).append(pd.DataFrame({'word_count': data.premise.apply(lambda x: len(x.split())),
                                                            'type' : 'premise'}))
        fig = px.histogram(wc, x='word_count', color='type', barmode='overlay', histnorm='density',
...
def label_count_langwise(data):
    ...
    if data.language.unique().size != 1:
        ...
        fig = px.pie(values=data.label.value_counts().values, 
                     names=data.label.value_counts().index, color_discrete_map={
                                 'entailment':'#a2c5ac',
                                 'neutral':'#9f87af',
                                 'contradiction':'#ef767a'})
        fig.update_traces(hoverinfo='label+percent', textinfo='percent', textfont_size=20,
                          marker=dict(colors=['#a2c5ac', '#9f87af', '#ef767a'], line=dict(color='#2f243a', width=2)))
...
def get_accs(validation, preds):
    ...
    tp_per_lang = validation[validation['label'] == preds].groupby('language').agg({'language': ['count']}).sort_index()
...
def model_report(data):
    ...
    fig = make_subplots(rows=2, cols=2, shared_yaxes=True, subplot_titles = [f'BERT accuracy by each epoch\n(trained on {data})',
                                                                             f'ROBERTA accuracy by each epoch\n(trained on {data})', 
                                                                             'BERT accuracy distribution by each language', 
                                                                             'ROBERTA accuracy distibution by each language'])

    fig.add_trace(go.Scatter(x=np.arange(len(bert['accuracy'])), y=bert['accuracy'],
                            mode='lines+markers',
                            line = dict(color='#2f243a', width=4),
                            name='train'),
                  row=1, col=1)

    fig.add_trace(go.Scatter(x=np.arange(len(bert['val_accuracy'])), y=bert['val_accuracy'],
                            mode='lines+markers',
                            name='validation',
                            line = dict(color='#ef767a', width=4)),
                  row=1, col=1)

    fig.add_trace(go.Scatter(x=np.arange(len(roberta['accuracy'])), y=roberta['accuracy'],
                            mode='lines+markers',
    ...
    fig.add_trace(go.Scatter(x=np.arange(len(roberta['val_accuracy'])), y=roberta['val_accuracy'],
                            mode='lines+markers',
                            name='validation',
                            showlegend=False,
    ...
    names, values = get_accs(validation, pd.read_csv(f'modeling\\predictions\\bert_{data}_preds.csv').label.apply(lambda x: {0: 'entailment', 
                                                                                   1: 'neutral', 
                                                                                   2: 'contradition'}[int(x)]))
    fig.add_trace(go.Bar(x=values, y=names, orientation='h', 
                         marker = dict(color='#9f87af', line=dict(color='black',width=0.2)), showlegend=False),
                         row=2, col=1)

    names, values = get_accs(validation, pd.read_csv(f'modeling\\predictions\\roberta_{data}_preds.csv').label.apply(lambda x: {0: 'entailment', 
                                                                                   1: 'neutral', 
                                                                                   2: 'contradition'}[int(x)]))
    fig.add_trace(go.Bar(x=values, y=names, orientation='h', 
                         marker = dict(color='#ef767a', line=dict(color='black',width=0.2)), showlegend=False),
                         row=2, col=2)
...
app.layout = html.Div([html.Div([
                            # 1 dash title, descr
                            html.H1(children='Natural Language Inference', style={'color': '#2f243a', 
                                                                                #  'fontSize': 14, 
                                                                                 'text-align': 'center'}),
                            html.H3(children='comprehensive task overview', style={'text-align': 'center',
                                                                              'color': '#444054'}),
                            html.P(children='Natural Language Inference (or Recognizing Textual Entailment), \
is the task of determining the inference relation between two (short, ordered) texts: \
entailment, neutral, or contradition', style={'text-align': 'center',
                                              'fontSize': 20,
                                              'color': '#2f243a'}),
                             html.P(children='Select an NLI corpus you would like to inspect', style={'text-align': 'center',
                                              'fontSize': 14,
                                              'color': '#2f243a'})
                            # 1 end      
                                ]),
                        html.Div([dcc.Dropdown(id='data', 
                                               options = [{'label': 'Stanford NLI', 'value': 'snli'},
                                                          {'label': 'Cross-lingual NLI', 'value': 'xnli'},
                                                          {'label': 'Multi-Genre NLI', 'value': 'mnli'}],
                                               value='xnli', 
                                               multi=False),
                                    html.Div([
                                    html.Div(dcc.Graph(figure=pie_by_langs('validation')),
                                             className='six columns'),
                                    html.Div(dcc.Graph(id='eda_langs'), className='six columns')],
                                    className='row'),

                                    html.Div([
                                    html.Div(dcc.Graph(figure=word_count_langwise('validation')),
                                             className='six columns'),
                                    html.Div(dcc.Graph(id='word_count'), className='six columns')],
                                    className='row'),

                                    html.Div([
                                    html.Div(dcc.Graph(figure=label_count_langwise('validation')),
                                             className='six columns'),
                                    html.Div(dcc.Graph(id='label_count'), className='six columns')],
                                    className='row')]), 
                        
                                    html.H3('BERT and XML-RoBERTa comparison', style={'text-align': 'center',
                                                                                       'color': '#444054'}),
